...
func (t *alertsTable) ListAlertsByName(name string) table.Table {
	...
	for _, vmAlert := range t.alerts.Data.Alerts {
		if vmAlert.Name == name {
			t.table.AddRow(vmAlert.Labels["alertname"], vmAlert.Annotations["Summary"], vmAlert.Annotations["Description"], vmAlert.State, vmAlert.ActiveAt)
		}
	}
...
func (t *alertsTable) ListAlertsByLabels(label, value string) table.Table {
	...
	for _, vmAlert := range t.alerts.Data.Alerts {
		...
		if ok && val == value {
			t.table.AddRow(vmAlert.Labels["alertname"], vmAlert.Annotations["Summary"], vmAlert.Annotations["Description"], vmAlert.State)
		}
	}
